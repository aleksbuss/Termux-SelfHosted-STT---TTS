...
def test_run_diagnostics_success():
    with patch("os.path.isfile", return_value=True), \
         patch("os.path.getsize", return_value=15000000), \
...
def test_run_diagnostics_piper_missing():
    ...
    with patch("os.path.isfile", side_effect=mock_isfile), patch("os.path.getsize", return_value=15000000), patch("os.access", return_value=True):
...
def test_run_diagnostics_ffmpeg_missing():
    with patch("os.path.isfile", return_value=True), patch("os.path.getsize", return_value=15000000), patch("os.access", return_value=True), patch("shutil.which", return_value=None):
...
def test_run_diagnostics_proot_warning():
    ...
    with patch("os.path.isfile", return_value=True), patch("os.path.getsize", return_value=15000000), patch("os.access", return_value=True), patch("shutil.which", side_effect=mock_which), patch("src.diagnostics.log.warning") as mock_warn:
        run_diagnostics()
