...
def run_diagnostics():
    ...
    if not os.path.isfile(WHISPER_MODEL):
        raise EnvironmentError(f"Whisper model not found: {WHISPER_MODEL}. Please run install.sh again.")
    ...
    for lang, cfg in LANG_CONFIG.items():
        ...
        if not os.path.isfile(model_path):
            raise EnvironmentError(f"Piper model for {lang} not found: {model_path}. Please run install.sh again.")
